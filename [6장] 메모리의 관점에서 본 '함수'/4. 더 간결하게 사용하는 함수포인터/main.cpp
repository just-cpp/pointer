...
// 콜백함수의 타입 재정의
typedef void (*STORE_FUNC)(FILE*, void*);


// 콜백함수 호출을 위해 전달되는 내용을 담을 구조체 선언
struct TCallback
{
    STORE_FUNC pWrite;                  // 호출될 콜백함수
    void* pData;                        // 콜백함수에 전달할 데이터
};
...
// pFName: 저장될 파일명
// pData: 저장될 데이터
// pCallback: 실제 저장 로직을 처리하는 콜백함수와 전달 데이터가 저장된 구조체의 주소
int32_t StoreInfo_ByCallback(const char* pFName, struct TCallback* pCallback)
{
    ...
    pCallback->pWrite(fp, pCallback->pData);
...
int32_t main(int32_t argc, char** argv)
{
    ...
    // 콜백함수 호출을 위한 구조체 변수 선언
    struct TCallback callback;

    // 호출될 콜백함수 설정
    callback.pWrite = WriteCar;
    // 저장될 데이터(car1)의 주소 설정
    callback.pData = (void*)&car1;
    // 저장함수 호출
    StoreInfo_ByCallback("car.dat", &callback);

    // 호출될 콜백함수 설정
    callback.pWrite = WriteBike;
    // 저장될 데이터(bike1)의 주소 설정
    callback.pData = (void*)&bike1;
    // 저장함수 호출
    StoreInfo_ByCallback("bike.dat", &callback);
